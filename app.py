...
def alarm_worker():
    while not alarm_stop.is_set():
        ...
        if alarm_stop.is_set():
            break
        try:
            # Note: playsound has issues in cloud deployments. 
            # For deployment, consider replacing this with a web-friendly audio solution 
            # like st.audio() or using streamlit-webrtc for camera access.
...
# Streamlit Controls
# -----------------------------
# Note: For deployment (e.g., Streamlit Community Cloud), 
# you should use streamlit-webrtc for live video stream handling 
# instead of cv2.VideoCapture(0) which accesses the server's webcam.
# The `playsound` library is also often problematic in a cloud environment.
